  ...
  <div class="home clear">
    <main class="fixed top0 width100" style="height: calc(100% - 45px)">
      <scroll-view scroll-y style="height: 100%;" @scrolltolower="loadMore">
        <header class="borderbottom1 flex-spacearound ts-bgcolor-bg" style="height: 40px">
          <a class="flex-align paddingX10" href="/pages/selectCity/main">
            {{curCity}}<i class="icon-uparrow rotate180 color-999"></i>
          </a>
          <div class="inputWrap ">
            <i class="icon-search "></i>
            <input class="searchInput" placeholder="找影院"  @tap="changePath('/pages/search/main')">
          </div>
...
<script>
  ...
  import { resFilmData, resCinemaData } from '@/utils/resData'
  export default {
    components: {
      filmList
    },
    ...
    async onLoad () {
      ...
      this.cinemaList.push(...resCinemaData)
